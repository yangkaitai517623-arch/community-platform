  ...
  <div class="admin-page">
    <header class="admin-page-header">
      <div class="admin-page-title-block">
        <span class="admin-page-eyebrow">跑腿订单 · ERRAND ORDERS</span>
        <h1 class="admin-page-title">跑腿订单管理</h1>
        <p class="admin-page-subtitle">查看已接单、进行中、已完成和已评价的跑腿服务订单</p>
      </div>
    ...
    <div class="admin-surface">
      <el-table :data="tableData" v-loading="loading">
        ...
        <el-table-column prop="errandType" label="类型" width="110">
          <template #default="{ row }"><span class="muted">{{ row.errandType || '-' }}</span></template>
        </el-table-column>
        <el-table-column prop="reward" label="报酬" width="100" align="right">
          <template #default="{ row }">
            <span class="admin-price">¥{{ row.reward }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="viewDialogVisible" title="跑腿订单详情" width="620px">
...
const getStatusText = (s) => ({ 1: '已接单', 2: '进行中', 3: '已完成' }[s] || '-')
